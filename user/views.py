...
from .forms import StudentSignUpForm, ProfessorSignUpForm, StudentUpdateForm, FeedbackForm
from .models import Feedback, Student, User
...
def home_view(request):
    ...
    return render(request, "home.html", context={"home": "active", "feedbacks": all_feedback})
...
def get_feedback(request):
    if request.method == "POST":
        form = FeedbackForm(data=request.POST, user=request.user)
        print("Form = ", form)
        if form.is_valid():
            form.save()
            return redirect('home')
        ...
        form = FeedbackForm()
    return render(request, "feedback.html", {"form": form, "home": "active"})

def admin_view_feedback(request):
    all_feedbacks = Feedback.objects.all()
    return render(request, "view_feedback.html", {"all_feedbacks": all_feedbacks})
